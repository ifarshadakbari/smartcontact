...
let departments = [
  ...
  { id: '4', name: 'فروش و بازاریابی', code: 'MKT', sort_order: 4 },
  { id: '5', name: 'پشتیبانی فنی و مشتریان', code: 'SUP', sort_order: 5 },
  { id: '6', name: 'تدارکات و بازرگانی', code: 'LOG', sort_order: 6 },
];

// Initial Seed Data for LDAP Domains
let ldapDomains = [
  {
    id: '1',
    ...
    encryption: 'none',
    bind_user: 'CN=Administrator,CN=Users,DC=parszarasa,DC=local',
    bind_password: '',
    user_filter: '(&(objectClass=user)(sAMAccountName={username}))',
    is_default: true,
    is_active: true,
    voip_enabled: true,
    voip_server_host: '192.168.10.25',
    voip_ami_port: 5038,
    voip_ami_username: 'phonebook_ami',
    voip_context: 'from-internal',
    voip_channel_tech: 'SIP',
...
let contacts: any[] = [
  {
    ...
    first_name: 'فرشاد',
    last_name: 'اکبری',
    job_title: 'مدیر فناوری اطلاعات',
    department: 'فناوری اطلاعات و ارتباطات',
    location: 'ساختمان مرکزی - طبقه ۳ - اتاق ۳۰۲',
    mobiles: ['09121112233'],
    is_mobile_public: true,
    personal_mobiles: [],
    landlines: [{ number: '101', title: 'داخلی مستقیم' }, { number: '02188001122', title: 'خط شهری مستقیم' }],
    email: 'f.akbari@parszarasa.local',
    description: 'مدیریت زیرساخت، شبکه، سرورها و سامانه‌های یکپارچه سازمانی',
    avatar: '',
    contact_type: 'internal',
    domain: 'parszarasa.local',
    domain_name: 'parszarasa.local',
    is_favorite: true,
    is_public: true,
  ...
  },
  {
    ...
    first_name: 'علی',
    last_name: 'رضایی',
    job_title: 'کارشناس ارشد شبکه و امنیت',
    department: 'فناوری اطلاعات و ارتباطات',
    location: 'اتاق سرور مرکزی',
    mobiles: ['09123334455'],
    is_mobile_public: true,
    personal_mobiles: [],
    landlines: [{ number: '102', title: 'داخلی شبکه' }],
    email: 'a.rezaei@parszarasa.local',
    description: 'پشتیبانی سوییچ‌ها، فایروال و ارتباطات امن',
    avatar: '',
    contact_type: 'internal',
    domain: 'parszarasa.local',
    domain_name: 'parszarasa.local',
  ...
  },
  {
    ...
    is_mobile_public: true,
    personal_mobiles: [],
    landlines: [{ number: '201', title: 'داخلی اداری' }],
  ...
  },
  {
    ...
    is_mobile_public: true,
    personal_mobiles: [],
    landlines: [{ number: '301', title: 'داخلی حسابداری' }],
  ...
  },
  {
    ...
    is_mobile_public: true,
    personal_mobiles: [],
    landlines: [{ number: '401', title: 'داخلی فروش' }],
  ...
  },
  {
    ...
    is_mobile_public: true,
    personal_mobiles: [],
    landlines: [{ number: '2020', title: 'مرکز تماس مخابرات' }, { number: '02188888888', title: 'پشتیبانی اختصاصی' }],
...
app.delete('/api/contacts/:id', (req, res) => {
  ...
  res.json({ message: 'مخاطب با موفقیت حذف گردید.' });
});
...
app.post('/api/domains/sync', (req, res) => {
...
app.post('/api/domains/test-ldap', (req, res) => {
...
// LDAP & Admin Login
app.post('/api/login/ldap', (req, res) => {
  ...
  const extension = matchedContact?.landlines?.[0]?.number || (isAdmin ? '101' : '');
...
app.post('/api/departments/sync', (req, res) => {
...
// VoIP Click-to-Call
app.post('/api/voip/originate', (req, res) => {
